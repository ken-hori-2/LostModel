...
class Algorithm_advance_re():
    ...
    def Advance_re(self, STATE_HISTORY, state, TRIGAR, Storage, OBS, total_stress, grid, CrossRoad, x):
            ...
            if self.map_unexp_area: #  or self.FIRST:
                ...
                # if not self.TRIGAR:
                    ...
                    if self.NODELIST[self.state.row][self.state.column] in pre:
                        ...
                        self.Storage.append(self.state)
                        ...
                        # # except:
                        # #     self.Storage = self.Storage.tolist()
                        # #     self.Storage_Stress.append = self.Storage_Stress.tolist()
                    ...
                    if self.total_stress >= PERMISSION[index][0]                         +x:
                        ...
                        self.COUNT += 1
                        self.Storage.append(self.state) # Arcを計算する為に、最初だけ必要
                        self.Add_Advance = True
        ...
        return self.total_stress, self.STATE_HISTORY, self.state, self.TRIGAR, self.OBS, self.Storage, self.action, self.Add_Advance, GOAL, self.SAVE_ARC, self.CrossRoad, self.Storage, self.Storage_Stress # , permission