...
import sys
sys.path.append("/Users/ken/Desktop/model/src/Oneroad/explore/agent/src/__Algorithm__NEW/3_road/Lost_Model/")
...
from Lost.bp import Algorithm_bp
...
from Lost.policy import Algorithm_exp
...
from environment_prob import State
...
def main():
    ...
    for test in range(1):
        ...
        for x in range(1): # 5):
            ...
            for i in range(1): # 20): # 4 戻るノードの個数以上は回す
                print("===================\n🐬🍏🍋 {} steps\n===================".format(i))
                ...
                print("\n============================\n🤖 🔛　アルゴリズム切り替え -> agent Back position\n============================")
                BPLIST = []
                test_state = state # [state.row-10, state.column]
                print("State : {}".format(test_state))
                STATE_HISTORY.append(test_state)
                STATE_HISTORY.append(test_state)
                STATE_HISTORY.append(test_state)
                
                BPLIST.append(State(22, 8))
                print("BPLIST : {}".format(BPLIST))
                total_stress, STATE_HISTORY, state, OBS, BackPosition_finish = back_position.BP(STATE_HISTORY, state, TRIGAR, OBS, BPLIST, total_stress) # action, Add_Advance, total_stress, SAVE_ARC)
                print("============\n=🤖　🌟　⚠️ =\n============")
